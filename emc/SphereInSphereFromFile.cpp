...
SphereInSphereFromFile::SphereInSphereFromFile(double r1, double r2, string phiFile) : r1(r1), r2(r2), Geometry(1.0)
{
	...
	printf("%s\n", title.c_str());
	printf("%d x %d array, spacing %.5fm\n\n", nRows, nCols, latticeSpacing);
}

double SphereInSphereFromFile::phiVertex(int i, int j) const
{
...
double SphereInSphereFromFile::Phi(const Vec3& r) const
{
	double rho = sqrt(r.x * r.x + r.y * r.y);
	return PhiRhoZ(rho, r.z);
}

double SphereInSphereFromFile::PhiRhoZ(double rho, double z) const
{
...
Vec3 SphereInSphereFromFile::EField(const Vec3& pos) const
{
...
Vec3 SphereInSphereFromFile::EFieldXZ(double x, double z) const
{
